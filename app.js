...
app.post('/log', (req, res) => {
	...
	req.on('end', () => {
		...
		MongoClient.connect(url, (err, db) => {
			...
			findDocuments(db, (doc) => {
				for(let i = 0; i < doc.length; i++) {
					if(doc[i].idCard === dataObj.idCard) { // 如果用户输入身份证在数据库中存在
						if(doc[i].password === dataObj.password) { // 如果密码正确则登陆成功
							db.close();
							res.json({ message: 'success' });
							userinfo = dataObj.idCard;
							console.log(getTime());
		...
		MongoClient.connect(url, (err, db) => {
			...
			findDocuments(db, (doc) => {
				...
				updateDocument(db, userinfo, totalMoney, () => {
					db.close();
					res.json({ message: +dataObj.money });
					console.log(getTime());
